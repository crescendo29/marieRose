...
<div class="relative z-[100] w-full h-[80px] shadow-xl flex justify-between items-center px-4 bg-gold-texture text-marieBlue md:justify-around">
  ...
  >
  <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 {!menuShow ? 'flex' : 'hidden'}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    <path stroke-linecap="round" stroke-linejoin="round" d="M4 6h16M4 12h16M4 18h16" />
  </svg>
  <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 {menuShow ? 'flex fixed top-[5%] right-[10%]' : 'hidden'}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
  ...
  <ul class={
    !menuShow
      ? "hidden"
      : "fixed top-0 left-0 w-full h-screen bg-gold-texture text-marieBlue flex flex-col justify-center items-center"
  }>
          <li class="py-6 text-xl" on:click={toggleNavbar}><a href="/">Accueil</a></li>
          <li class="py-6 text-xl" on:click={toggleNavbar}><a href="/prestations">Nos Prestations</a></li>
          <li class="py-6 text-xl" on:click={toggleNavbar}><a href="/produits">Nos Produits</a></li>
          <li class="py-6 text-xl" on:click={toggleNavbar}><a href="https://www.planity.com/douce-evasion-29300-quimperle">Prendre Rendez-Vous</a></li>
          <li class="py-6 text-xl" on:click={toggleNavbar}><a href="/espacecadeaux">Espace Cadeaux</a></li>
          <li class="py-6 text-xl" on:click={toggleNavbar}><a href="/infos">Infos Pratiques</a></li>
          <li class="py-6 text-xl" on:click={toggleNavbar}>
